...
const AlertsPage = ({ alerts, onResolveAlert }) => {
  ...
  const displayedAlerts = showAllHistory ? filteredAlerts : filteredAlerts.slice(0, 6);
        ...
        {filteredAlerts.length === 0 ? (
          ...
          <div className="space-y-4">
            ...
            {filteredAlerts.length > 6 && (
              <div className="pt-4 border-t border-slate-800/80 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400 font-mono">
                <span>
                  {showAllHistory 
                    ? `Showing all ${filteredAlerts.length} alert logs` 
                    : `Showing 6 recent alerts out of ${filteredAlerts.length} total logs`}
                </span>
                <button
                  onClick={() => setShowAllHistory(!showAllHistory)}
                  className="px-4 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-cyan-400 border border-slate-700 font-semibold transition flex items-center space-x-2 shadow-sm"
                >
                  <span>{showAllHistory ? "🔼 Show Recent Alerts Only (6)" : `📜 View Full History (${filteredAlerts.length} Logs)`}</span>
                </button>
              </div>
            )}
          </div>
...
const App = () => {
  ...
  const fetchTelemetry = React.useCallback(async () => {
    try {
      const telemetry = await window.ApiService.getUnifiedTelemetry(selectedMachineId);
      if (telemetry) {
        setIsUsingFallback(false);
        if (telemetry.machines && telemetry.machines.length) setMachines(telemetry.machines);
        if (telemetry.current_machine) setCurrentMachine(telemetry.current_machine);
        if (telemetry.sensors) setSensorsData(telemetry.sensors);
        if (telemetry.health) setHealthData(telemetry.health);
        if (telemetry.prediction) setPredictionData(telemetry.prediction);
        if (telemetry.alerts) setAlertsData(telemetry.alerts);
        if (telemetry.analytics) setAnalyticsData(telemetry.analytics);
        if (telemetry.maintenance) setMaintenanceData(telemetry.maintenance);
      } else {
